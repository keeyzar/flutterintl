...
class GPTTranslationRequestClient(
    ...
    suspend fun requestTranslation(content: String, targetLanguage: String): String {
        ...
        val history: List<Content> = listOf(
        ...
        val response = gemini.models.generateContent(model, history, GenerateContentConfig.builder()
            .systemInstruction(Content.builder().parts(Part.builder().text("You are a JSON Api Translator. You are given a JSON file and a target language. Your task is to translate the content of the JSON file to the target language, even if it is the same language.")).build()).build())
    ...
    private suspend fun requestComplexTranslation(content: String, targetLanguage: String): String {
        ...
        val history = listOf(
            Content.builder().role("user").parts(Part.builder().text(initialRequest).build()).build(),
            Content.builder().role("model").parts(Part.builder().text(initialAnswer).build()).build(),
            Content.builder().role("user").parts(Part.builder().text(secondRequest).build()).build(),
            Content.builder().role("model").parts(Part.builder().text(secondResponse).build()).build(),
            Content.builder().role("user").parts(Part.builder().text(thirdRequest).build()).build(),
            Content.builder().role("model").parts(Part.builder().text(thirdResponse).build()).build(),
            Content.builder().role("user").parts(Part.builder().text(realRequest).build()).build()
        )
        ...
        val model = "models/gemini-2.5-flash"
        val response = gemini.models.generateContent(model, history, GenerateContentConfig.builder()
            .systemInstruction(Content.builder().parts(Part.builder().text("You are responsible for internationalization of flutter strings. Internationalization can be done with or without plural.\n" +
                    "Types are: String, num(with format compact), DateTime (with format, e.g. yMd)")).build()).build())
        return response.text() ?: throw Exception("Could not translate content")
    }
    ...
    suspend fun requestComplexTranslationLong(content: String, targetLanguage: String): String {
        ...
        val initialRequest = """
            ...
            the type is either num or string, if string, no format is required
        ...
        val model = "models/gemini-2.5-flash"
        val response = gemini.models.generateContent(model, history, GenerateContentConfig.builder()
            .systemInstruction(Content.builder().parts(Part.builder().text("""
